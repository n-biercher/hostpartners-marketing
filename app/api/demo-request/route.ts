...
export async function POST(request: NextRequest) {
  ...
  const requestedTimezone = typeof body.requestedTimezone === "string" ? body.requestedTimezone : "Europe/Berlin"
  ...
  try {
    ...
    if (!isTurnstileValid) {
      return json({ error: "Sicherheitsprüfung fehlgeschlagen. Bitte erneut versuchen." }, 400)
    }
  } catch (error) {
    const message = error instanceof Error ? error.message : "Turnstile-Prüfung fehlgeschlagen."
  ...
  if (!ingestResponse.ok) {
    ...
    return json({ error: typeof data?.error === "string" ? data.error : "Demo-Anfrage konnte nicht verarbeitet werden." }, ingestResponse.status)
  }
